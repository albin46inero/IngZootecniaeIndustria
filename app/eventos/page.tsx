...
import { ArrowLeft, Calendar, MapPin, Clock, Search, ChevronLeft, ChevronRight, X, ZoomIn, Sparkles, Mail, AlertCircle } from 'lucide-react';
...
export default function EventosPage() {
  ...
  const isEventosInView = useInView(eventosRef, { once: true, margin: '-100px' });
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (gacetaResponse.upea_evento && gacetaResponse.upea_evento.length > 0) {
          setEventos(gacetaResponse.upea_evento);
          setFilteredEventos(gacetaResponse.upea_evento);
          console.log('✅ Eventos cargados:', gacetaResponse.upea_evento.length);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (selectedTipo !== 'all') {
      filtered = filtered.filter(e => e.tipo_evento.toLowerCase() === selectedTipo.toLowerCase());
    }
    
    if (searchTerm) {
      filtered = filtered.filter(e => 
        e.evento_titulo.toLowerCase().includes(searchTerm.toLowerCase()) ||
        e.evento_descripcion.toLowerCase().includes(searchTerm.toLowerCase()) ||
        e.tipo_evento.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    
    setFilteredEventos(filtered);
  }, [searchTerm, selectedTipo, eventos]);
  ...
  const getUniqueTypes = () => {
    const types = new Set(eventos.map(e => e.tipo_evento));
      ...
      <div className="min-h-screen bg-background font-forte">
        ...
        >
          ...
          >
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3">Eventos Académicos</h2>
            <p className="text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto">
              Participa en nuestros eventos, actividades académicas y experiencias educativas de la carrera de <span className="font-semibold" style={{ color: colors.primario }}>{institucionData?.institucion_nombre || 'Ciencias del Desarrollo'}</span>
            </p>
          ...
          {filteredEventos.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
              {filteredEventos.map((evento, index) => {
                const isBroken = brokenImages.has(evento.evento_id.toString());
                ...
                return (
                  <motion.div 
                    ...
                    className="bg-white rounded-2xl shadow-sm border border-border overflow-hidden transition-all duration-300 group cursor-pointer"
                    onClick={() => openModal(evento)}
                  >
                    {/* 🔹 IMAGEN CON MANEJO DE ERRORES */}
                    <div className="relative h-56 overflow-hidden bg-gradient-to-br from-[var(--color-primario)]/10 to-[var(--color-secundario)]/10">
                      {evento.evento_imagen && !isBroken ? (
                        <motion.div whileHover={{ scale: 1.1 }} transition={{ duration: 0.6 }}>
                          <Image
                            src={getDirectUrl(evento.evento_imagen)}
                            alt={evento.evento_titulo}
                            fill
                            className="object-cover transition-transform duration-700"
                            onError={() => {
                              console.error('❌ Imagen de evento no encontrada:', {
                                id: evento.evento_id,
                                titulo: evento.evento_titulo,
                                path: evento.evento_imagen
                              });
                              setBrokenImages(prev => new Set(prev).add(evento.evento_id.toString()));
                            }}
                          />
                        </motion.div>
                      ) : (
                        <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200">
                          {evento.evento_imagen ? (
                            ...
                            <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: 'linear' }}>
                              <Calendar className="w-16 h-16" style={{ color: colors.primario }} />
                            </motion.div>
                          )}
                        </div>
                      )}
                      ...
                      <div className="space-y-2 text-sm text-muted-foreground">
                        ...
                        {evento.evento_hora && (
                          <motion.div className="flex items-center gap-2" whileHover={{ x: 3 }}>
                            ...
                            <span>{formatTime(evento.evento_hora)}</span>
                          </motion.div>
                        )}
                        {evento.evento_lugar && (
                          <motion.div className="flex items-center gap-2" whileHover={{ x: 3 }}>
            ...
            >
              <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: 'linear' }} className="w-16 h-16 text-muted-foreground/50 mx-auto mb-4">
                <Calendar className="w-full h-full" />
              </motion.div>
              <p className="text-muted-foreground text-lg">
                {searchTerm ? 'No se encontraron eventos.' : 'No hay eventos disponibles en este momento.'}
              </p>
            </motion.div>
          )}
          ...
          {selectedEvento && (
            ...
            >
              ...
              >
                ...
                {/* Imagen */}
                {!brokenImages.has(selectedEvento.evento_id.toString()) && selectedEvento.evento_imagen && (
                  <motion.div 
                    className="relative h-64 md:h-96 bg-gradient-to-br from-[var(--color-primario)]/10 to-[var(--color-secundario)]/10"
                    whileHover={{ scale: 1.02 }}
                    transition={{ duration: 0.4 }}
                  >
                    <Image
                      src={getDirectUrl(selectedEvento.evento_imagen)}
                      alt={selectedEvento.evento_titulo}
                      fill
                      className="object-cover"
                      onError={() => {
                        console.error('❌ Imagen modal no encontrada:', selectedEvento.evento_imagen);
                        setBrokenImages(prev => new Set(prev).add(selectedEvento.evento_id.toString()));
                      }}
                    />
                ...
                <div className="p-6 md:p-8">
                  ...
                  >
                    {selectedEvento.evento_titulo}
                  </motion.h2>
                  
                  {/* Detalles */}
                  <motion.div 
                    className="grid md:grid-cols-3 gap-4 mb-6 p-4 bg-muted/50 rounded-xl"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    transition={{ delay: 0.3 }}
                  >
                    {selectedEvento.evento_fecha && (
                      <motion.div className="flex items-center gap-2 text-sm" whileHover={{ scale: 1.05 }}>
                        ...
                        <div>
                          ...
                          <p className="text-muted-foreground">{formatDate(selectedEvento.evento_fecha)}</p>
                        </div>
                      </motion.div>
                    )}
                    {selectedEvento.evento_hora && (
                      <motion.div className="flex items-center gap-2 text-sm" whileHover={{ scale: 1.05 }}>
                        ...
                        <div>
                          ...
                          <p className="text-muted-foreground">{formatTime(selectedEvento.evento_hora)}</p>
                        </div>
                      </motion.div>
                    )}
                    {selectedEvento.evento_lugar && (
                      <motion.div className="flex items-center gap-2 text-sm" whileHover={{ scale: 1.05 }}>
                        <motion.div animate={{ y: [0, -3, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 1 }}>
                          <MapPin className="w-4 h-4" style={{ color: colors.terciario }} />
                        </motion.div>
                        <div>
                          <p className="font-semibold text-foreground">Lugar</p>
                          <p className="text-muted-foreground">{selectedEvento.evento_lugar}</p>
                        </div>
                      </motion.div>
                    )}
                  </motion.div>
                  ...
                  <motion.div 
                    ...
                    dangerouslySetInnerHTML={{ __html: selectedEvento.evento_descripcion }}
                  />
                  ...
                  >
                    {!brokenImages.has(selectedEvento.evento_id.toString()) && selectedEvento.evento_imagen && (
                      <motion.a 
                        href={getDirectUrl(selectedEvento.evento_imagen)}
                        target="_blank"
                      ...
                      >
                        <ZoomIn className="w-5 h-5" /> Ver Imagen
                      </motion.a>
                    )}
                    <motion.button 
                      onClick={closeModal}
