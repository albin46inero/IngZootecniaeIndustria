...
import { ArrowLeft, BookOpen, Award, ChevronLeft, ChevronRight, Sparkles, Mail, MapPin, Phone, Calendar, Users, GraduationCap, AlertCircle } from 'lucide-react';
...
export default function HistoriaPage() {
  ...
  const isHistoriaInView = useInView(historiaRef, { once: true, margin: '-100px' });
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Fallback para portadas
        setPortadas([
          {
            portada_id: 0,
            portada_imagen: 'default.jpg',
            portada_titulo: 'Historia de la Carrera',
  ...
  // 🔹 Auto-rotación del slider (cada 8 segundos)
  useEffect(() => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-white">
        ...
        >
          ...
          />
          <p className="text-muted-foreground font-medium">Cargando historia...</p>
        </motion.div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background font-forte overflow-x-hidden">
        ...
        >
          ...
          <motion.section 
            ...
            className="mb-16"
          >
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 md:p-12 hover:shadow-2xl transition-shadow">
              <motion.div 
              ...
              >
                <motion.div 
                  ...
                  transition={{ duration: 0.6 }}
                >
                  <BookOpen className="w-6 h-6" style={{ color: colors.primario }} />
                </motion.div>
              ...
              <motion.div 
                ...
                dangerouslySetInnerHTML={{ 
                  __html: institucionData?.institucion_historia || '<p>Información no disponible</p>' 
                }}
              />
            </div>
          </motion.section>

          {/* Información Institucional con animación */}
          <motion.section 
            initial={{ opacity: 0, y: 40 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              {institucionData?.institucion_nombre && (
                ...
                >
                  ...
                  <div>
                    <h4 className="font-semibold text-foreground mb-1">Nombre de la Carrera</h4>
                    <p className="text-muted-foreground font-forte">{institucionData.institucion_nombre}</p>
                  </div>
                </motion.div>
              )}
              ...
              {institucionData?.institucion_iniciales && (
                ...
                >
                  <motion.div 
                    ...
                    whileHover={{ rotate: -10 }}
                  >
                    <BookOpen className="w-5 h-5" style={{ color: colors.secundario }} />
                  </motion.div>
                  <div>
                    <h4 className="font-semibold text-foreground mb-1">Siglas</h4>
                    <p className="text-muted-foreground font-forte">{institucionData.institucion_iniciales}</p>
                  </div>
                </motion.div>
              )}
              ...
              {institucionData?.institucion_direccion && (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground font-forte">{institucionData.institucion_direccion}</p>
                  </div>
                </motion.div>
              )}
              ...
              {institucionData?.institucion_correo1 && (
                ...
                >
                  ...
                  >
                    <Mail className="w-5 h-5" style={{ color: colors.secundario }} />
                  </motion.div>
                  <div>
              ...
              {institucionData?.institucion_celular1 && institucionData.institucion_celular1 !== 2147483647 && (
                ...
                >
                  <motion.div 
                    ...
                    whileHover={{ rotate: -15 }}
                  >
                    <Phone className="w-5 h-5" style={{ color: colors.terciario }} />
                  </motion.div>
                  <div>
          ...
          {/* Timeline visual de hitos (opcional, se puede personalizar) */}
          <motion.section 
            initial={{ opacity: 0, y: 40 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          ...
          >
            ...
            <div className="relative">
              {/* Línea vertical */}
              <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-[var(--color-primario)] to-[var(--color-secundario)]"></div>
              
              {/* Items del timeline */}
              <div className="space-y-8">
                {[
                  { year: '2010', title: 'Fundación', desc: 'Creación de la carrera de Ciencias del Desarrollo' },
                  { year: '2015', title: 'Acreditación', desc: 'Primera acreditación institucional' },
                  { year: '2020', title: 'Expansión', desc: 'Nuevos laboratorios y convenios internacionales' },
                  { year: '2024', title: 'Innovación', desc: 'Implementación de tecnologías educativas' }
                ].map((hito, index) => (
                  <motion.div 
                    key={index}
                  ...
                  >
                    <div className={`flex-1 ${index % 2 === 0 ? 'md:text-right' : 'md:text-left'} pl-16 md:pl-0`}>
                      ...
                      >
                        ...
                        <h4 className="font-bold text-foreground mb-2">{hito.title}</h4>
                        <p className="text-sm text-muted-foreground font-forte">{hito.desc}</p>
                      </motion.div>
                    </div>
                    
                    {/* Punto del timeline */}
                    <motion.div 
                      className="absolute left-8 md:left-1/2 w-4 h-4 rounded-full border-4 border-white shadow-lg"
