...
import { ArrowLeft, Calendar, MapPin, Clock, Users, DollarSign, MessageCircle, Search, ChevronLeft, ChevronRight, GraduationCap, X, ZoomIn, Sparkles, Mail, AlertCircle, FileText, ExternalLink, Download } from 'lucide-react';
...
export default function SeminariosPage() {
  ...
  const [selectedSeminario, setSelectedSeminario] = useState<Curso | null>(null);
  const [brokenImages, setBrokenImages] = useState<Set<string>>(new Set());
  
  // 🔹 Referencias para animaciones
  const heroRef = useRef<HTMLDivElement>(null);
  const seminariosRef = useRef<HTMLDivElement>(null);
  ...
  const isSeminariosInView = useInView(seminariosRef, { once: true, margin: '-100px' });

  // 🔹 Cargar datos desde gacetaEventos
  useEffect(() => {
    const loadData = async () => {
      try {
        console.log('🔹 Cargando seminarios...');
        ...
        if (gacetaResponse.cursos && gacetaResponse.cursos.length > 0) {
          ...
          const seminariosFiltrados = gacetaResponse.cursos.filter(
            (curso: Curso) => 
              curso.tipo_curso_otro?.tipo_conv_curso_nombre === "SEMINARIOS" &&
              curso.det_estado === "1"
          );
          setSeminarios(seminariosFiltrados);
          console.log('✅ Seminarios cargados:', seminariosFiltrados.length);
        }
      } catch (error) {
  ...
  // 🔹 Abrir modal
  const openModal = (seminario: Curso) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-white">
        ...
        >
          ...
          />
          <p className="text-muted-foreground font-medium">Cargando seminarios...</p>
        </motion.div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background font-forte overflow-x-hidden">
        ...
        >
          ...
          >
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3">Seminarios Académicos</h2>
            <p className="text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto">
              Participa en nuestros seminarios especializados y actualiza tus conocimientos de la carrera de <span className="font-semibold" style={{ color: colors.primario }}>{institucionData?.institucion_nombre || 'Ciencias del Desarrollo'}</span>
            </p>
          ...
          {/* Grid de Seminarios con animaciones stagger */}
          {seminarios.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
              {seminarios.map((seminario, index) => {
                const isBroken = brokenImages.has(seminario.iddetalle_cursos_academicos.toString());
                ...
                return (
                  <motion.div 
                    ...
                    onClick={() => openModal(seminario)}
                  >
                    {/* 🔹 IMAGEN DEL SEMINARIO CON MANEJO DE ERRORES */}
                    <div className="relative h-48 overflow-hidden bg-gradient-to-br from-[var(--color-primario)]/10 to-[var(--color-secundario)]/10 group-hover:scale-105 transition-transform duration-300">
                      {seminario.det_img_portada && !isBroken ? (
                        <motion.div whileHover={{ scale: 1.1 }} transition={{ duration: 0.6 }}>
                          <Image
                            src={getDirectUrl(seminario.det_img_portada)}
                            alt={seminario.det_titulo}
                            fill
                            className="object-cover transition-transform duration-700"
                            onError={() => {
                              console.error('❌ Imagen de seminario no encontrada:', {
                                id: seminario.iddetalle_cursos_academicos,
                                titulo: seminario.det_titulo,
                                path: seminario.det_img_portada
                              });
                              setBrokenImages(prev => new Set(prev).add(seminario.iddetalle_cursos_academicos.toString()));
                            }}
                          />
                        </motion.div>
                      ) : (
                    ...
                    <div className="p-6">
                      ...
                      <div className="space-y-2 mb-6 pt-4 border-t border-gray-200 text-sm">
                        {seminario.det_carga_horaria > 0 && (
                          <motion.div className="flex items-center gap-2 text-muted-foreground" whileHover={{ x: 3 }}>
                            <Clock className="w-4 h-4" style={{ color: colors.primario }} />
                            <span className="font-forte">{seminario.det_carga_horaria} horas</span>
                          </motion.div>
                        )}
                        {seminario.det_modalidad && (
                          <motion.div className="flex items-center gap-2 text-muted-foreground" whileHover={{ x: 3 }}>
                            <MapPin className="w-4 h-4" style={{ color: colors.secundario }} />
                            <span className="font-forte">{seminario.det_modalidad}</span>
                          </motion.div>
                        )}
                        {seminario.det_fecha_ini && (
                          <motion.div className="flex items-center gap-2 text-muted-foreground" whileHover={{ x: 3 }}>
                            <Calendar className="w-4 h-4" style={{ color: colors.terciario }} />
                            <span className="font-forte">{formatDate(seminario.det_fecha_ini)}</span>
                          </motion.div>
                        )}
                        {seminario.det_lugar_curso && seminario.det_lugar_curso !== '---' && (
                          <motion.div className="flex items-center gap-2 text-muted-foreground" whileHover={{ x: 3 }}>
                            <MapPin className="w-4 h-4" style={{ color: colors.secundario }} />
                            <span className="line-clamp-1 font-forte">{seminario.det_lugar_curso}</span>
                          </motion.div>
                        )}
                        <motion.div className="flex items-center gap-2 text-muted-foreground" whileHover={{ x: 3 }}>
                          <DollarSign className="w-4 h-4" style={{ color: colors.primario }} />
                          <span className="font-forte">Bs. {seminario.det_costo}</span>
                        </motion.div>
                      </div>
                      
                      {seminario.det_grupo_whatssap && (
                        <motion.a
                          onClick={(e) => e.stopPropagation()}
                          href={seminario.det_grupo_whatssap.startsWith('http') ? seminario.det_grupo_whatssap : `https://wa.me/591${seminario.det_grupo_whatssap}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          whileHover={{ scale: 1.02 }}
                          whileTap={{ scale: 0.98 }}
                          className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-green-600 text-white font-semibold rounded-xl shadow-lg hover:bg-green-700 transition"
                        >
            ...
            >
              ...
              <p className="text-muted-foreground text-lg font-forte">
                {searchTerm ? 'No se encontraron seminarios.' : 'No hay seminarios disponibles en este momento.'}
              </p>
            </motion.div>
          )}
        ...
        {/* ===== MODAL PARA VER SEMINARIO - IGUAL QUE CONVOCATORIAS ===== */}
        <AnimatePresence>
          {selectedSeminario && (
            <motion.div 
            ...
            >
              ...
              >
                ...
                {!brokenImages.has(selectedSeminario.iddetalle_cursos_academicos.toString()) && selectedSeminario.det_img_portada && (
                  <motion.div 
                  ...
                  >
                    <Image
                      src={getDirectUrl(selectedSeminario.det_img_portada)}
                      alt={selectedSeminario.det_titulo}
                      fill
                      className="object-contain md:object-cover p-4 md:p-0"
                      onError={() => {
                        console.error('❌ Imagen modal no encontrada:', selectedSeminario.det_img_portada);
                        setBrokenImages(prev => new Set(prev).add(selectedSeminario.iddetalle_cursos_academicos.toString()));
                      }}
                    />
                  </motion.div>
                )}
                ...
                <div className="p-6 md:p-8">
                  ...
                  >
                    {selectedSeminario.tipo_curso_otro?.tipo_conv_curso_nombre || 'SEMINARIO'}
                  </motion.span>
                  ...
                  >
                    {selectedSeminario.det_titulo}
                  </motion.h2>
                  
                  {/* Detalles */}
                  <motion.div 
                    className="flex flex-wrap gap-4 text-sm text-muted-foreground mb-6 pb-4 border-b border-gray-200"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    transition={{ delay: 0.3 }}
                  >
                    {selectedSeminario.det_carga_horaria > 0 && (
                      <motion.div className="flex items-center gap-2" whileHover={{ scale: 1.05 }}>
                        <Clock className="w-4 h-4" style={{ color: colors.primario }} />
                        <span className="font-forte">{selectedSeminario.det_carga_horaria} horas</span>
                      </motion.div>
                    )}
                    {selectedSeminario.det_modalidad && (
                      <motion.div className="flex items-center gap-2" whileHover={{ scale: 1.05 }}>
                        <MapPin className="w-4 h-4" style={{ color: colors.secundario }} />
                        <span className="font-forte">{selectedSeminario.det_modalidad}</span>
                      </motion.div>
                    )}
                    {selectedSeminario.det_fecha_ini && (
                      <motion.div className="flex items-center gap-2" whileHover={{ scale: 1.05 }}>
                        <Calendar className="w-4 h-4" style={{ color: colors.terciario }} />
                        <span className="font-forte">{formatDate(selectedSeminario.det_fecha_ini)}</span>
                      </motion.div>
                    )}
                    {selectedSeminario.det_lugar_curso && selectedSeminario.det_lugar_curso !== '---' && (
                      <motion.div className="flex items-center gap-2" whileHover={{ scale: 1.05 }}>
                        <MapPin className="w-4 h-4" style={{ color: colors.secundario }} />
                        <span className="font-forte">{selectedSeminario.det_lugar_curso}</span>
                      </motion.div>
                    )}
                    <motion.div className="flex items-center gap-2" whileHover={{ scale: 1.05 }}>
                      <DollarSign className="w-4 h-4" style={{ color: colors.primario }} />
                      <span className="font-forte">Bs. {selectedSeminario.det_costo}</span>
                    </motion.div>
                  </motion.div>
                  ...
                  <motion.div 
                    ...
                    dangerouslySetInnerHTML={{ __html: selectedSeminario.det_descripcion }}
                  />
                  ...
                  <motion.div 
                    ...
                    transition={{ delay: 0.5 }}
                  >
                    {selectedSeminario.det_grupo_whatssap && (
                      <motion.a
                        href={selectedSeminario.det_grupo_whatssap.startsWith('http') ? selectedSeminario.det_grupo_whatssap : `https://wa.me/591${selectedSeminario.det_grupo_whatssap}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.98 }}
                    ...
                    {!brokenImages.has(selectedSeminario.iddetalle_cursos_academicos.toString()) && selectedSeminario.det_img_portada && (
                      <motion.a
                        href={getDirectUrl(selectedSeminario.det_img_portada)}
                        target="_blank"
                        rel="noopener noreferrer"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.98 }}
