...
import { ArrowLeft, FileText, Download, ChevronLeft, ChevronRight, X, ZoomIn, Maximize2, Eye, Sparkles, Calendar, Mail, AlertCircle } from 'lucide-react';
...
export default function PlanEstudiosPage() {
  ...
  const [planes, setPlanes] = useState<Gaceta[]>([]);
  const [currentSlide, setCurrentSlide] = useState(0);
  const [loading, setLoading] = useState(true);
  const [selectedPlan, setSelectedPlan] = useState<Gaceta | null>(null);
  ...
  // 🔹 Estado para rastrear previews de PDF rotos
  ...
  const isPlanesInView = useInView(planesRef, { once: true, margin: '-100px' });

  useEffect(() => {
    const loadData = async () => {
      try {
        console.log('🔹 Cargando plan de estudios...');
        ...
        if (gacetaResponse.upea_gaceta_universitaria && gacetaResponse.upea_gaceta_universitaria.length > 0) {
          const planesFiltrados = gacetaResponse.upea_gaceta_universitaria.filter(
            (g: Gaceta) => 
              g.gaceta_tipo.toUpperCase() === "PLAN" ||
              g.gaceta_titulo.toUpperCase().includes("PLAN DE ESTUDIOS")
          );
          setPlanes(planesFiltrados);
          console.log('✅ Planes cargados:', planesFiltrados.length);
        }
      } catch (error) {
  ...
  const openPDFViewer = (plan: Gaceta) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-white">
        ...
        >
          ...
          />
          <p className="text-muted-foreground font-medium">Cargando plan de estudios...</p>
        </motion.div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background font-forte overflow-x-hidden">
        ...
        >
          ...
          >
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3">Plan de Estudios</h2>
            <p className="text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto">
              Conoce la estructura académica y las asignaturas de la carrera de <span className="font-semibold" style={{ color: colors.primario }}>{institucionData?.institucion_nombre || 'Ciencias del Desarrollo'}</span>
            </p>
            <div className="w-20 h-1 bg-gradient-to-r from-[var(--color-primario)] to-[var(--color-secundario)] mx-auto mt-4 rounded-full"></div>
          </motion.div>

          {/* Grid de Planes con animaciones stagger */}
          {planes.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
              {planes.map((plan, index) => {
                const isBroken = brokenPreviews.has(plan.gaceta_id.toString());
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative h-64 overflow-hidden bg-gradient-to-br from-[var(--color-primario)]/10 to-[var(--color-secundario)]/10 group">
                      ...
                      {!isBroken ? (
                        <div className="absolute inset-0">
                          <iframe
                            src={`https://docs.google.com/viewer?url=${encodeURIComponent(getDirectUrl(plan.gaceta_documento))}&embedded=true`}
                            className="w-full h-full border-0"
                            title={plan.gaceta_titulo}
                            sandbox="allow-same-origin allow-scripts allow-popups"
                            onError={() => {
                              console.error('❌ Error cargando preview de PDF:', plan.gaceta_documento);
                              setBrokenPreviews(prev => new Set(prev).add(plan.gaceta_id.toString()));
                            }}
                          />
                        </div>
                      ) : (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center gap-3 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <motion.a
                          href={getDirectUrl(plan.gaceta_documento)}
                          download
                          target="_blank"
                    ...
                    <div className="p-6">
                      <motion.h3 
                        className="text-xl font-bold text-gray-900 mb-3 line-clamp-2 group-hover:text-[var(--color-primario)] transition-colors font-forte"
                        whileHover={{ x: 5 }}
                      >
                        {plan.gaceta_titulo}
                      </motion.h3>
                      ...
                      >
                        ...
                        <span className="font-forte">{formatDate(plan.gaceta_fecha)}</span>
                      </motion.div>
                    </div>
                  </motion.div>
                );
            ...
            >
              ...
              <p className="text-muted-foreground text-lg font-forte">No hay plan de estudios disponible en este momento.</p>
            </motion.div>
          )}
            ...
            >
              ...
              >
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-bold truncate font-forte">{selectedPlan.gaceta_titulo}</h3>
                    <p className="text-sm opacity-75 font-forte">{formatDate(selectedPlan.gaceta_fecha)}</p>
                  </div>
                </div>
                
                <div className="flex items-center gap-2">
                  <motion.a
                    href={getDirectUrl(selectedPlan.gaceta_documento)}
                    target="_blank"
                    rel="noopener noreferrer"
              ...
              >
                <div className="min-h-full flex items-center justify-center p-4">
                  ...
                  >
                    <iframe
                      src={`${getDirectUrl(selectedPlan.gaceta_documento)}#toolbar=1&navpanes=0&scrollbar=1`}
                      className="w-full h-[80vh] border-0"
                      title={selectedPlan.gaceta_titulo}
                    />
                  </motion.div>
                </div>
              </motion.div>
