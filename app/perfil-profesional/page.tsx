...
import { ArrowLeft, Award, ChevronLeft, ChevronRight, Briefcase, Target, Users, Sparkles, Mail, MapPin, Phone, AlertCircle } from 'lucide-react';
...
export default function PerfilProfesionalPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        console.log('🔹 Cargando perfil profesional...');
        
        // Cargar institución (incluye institucion_sobre_ins = perfil profesional)
        ...
        setPortadas([
          {
            ...
            portada_titulo: 'Perfil Profesional',
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-white">
        ...
        >
          ...
          />
          <p className="text-muted-foreground font-medium">Cargando perfil profesional...</p>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-background font-forte overflow-x-hidden">
        ...
        {/* ===== CONTENIDO PRINCIPAL - PERFIL PROFESIONAL ===== */}
        <motion.main 
          ref={contenidoRef}
          className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16"
        >
          {/* Perfil Profesional - Contenido desde API */}
          {institucionData?.institucion_sobre_ins ? (
            <motion.section 
              ...
              className="mb-16"
            >
              <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden hover:shadow-2xl transition-all duration-300 p-8 md:p-12">
                <motion.div 
                ...
                >
                  <motion.div 
                    ...
                    transition={{ duration: 0.6 }}
                  >
                    <Award className="w-7 h-7" style={{ color: colors.primario }} />
                  </motion.div>
                  <motion.h2 
                    className="text-3xl font-bold text-foreground font-forte"
                ...
                <motion.div 
                  ...
                  dangerouslySetInnerHTML={{ 
                    __html: institucionData.institucion_sobre_ins 
                  }}
                />
              </div>
            </motion.section>
            ...
            >
              <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden hover:shadow-2xl transition-all duration-300 p-8 md:p-12">
                ...
                <motion.p 
                  className="text-muted-foreground font-forte"
                  initial={{ opacity: 0 }}
                  whileInView={{ opacity: 1 }}
                  viewport={{ once: true }}
                  transition={{ delay: 0.3 }}
                >
                  El estudiante a la conclusión de sus estudios habrá adquirido conocimientos en:
                </motion.p>
                
                <motion.ul 
                  className="mt-6 space-y-3 text-muted-foreground font-forte"
                  initial={{ opacity: 0 }}
                  whileInView={{ opacity: 1 }}
                  viewport={{ once: true }}
                ...
                >
                  <motion.li 
                    ...
                    whileHover={{ x: 5 }}
                  >
                    <Target className="w-5 h-5 flex-shrink-0 mt-1" style={{ color: colors.primario }} />
                    <span>Aplicar el proceso administrativo por medio de estrategias y herramientas conceptuales, metodológicas y técnicas con una visión prospectiva global.</span>
                  </motion.li>
                  <motion.li 
                    className="flex items-start gap-3"
                    whileHover={{ x: 5 }}
                  >
                    <Target className="w-5 h-5 flex-shrink-0 mt-1" style={{ color: colors.primario }} />
                    <span>Implementar el sistema de calidad total y mejora continua para la optimización de los recursos.</span>
                  </motion.li>
                  <motion.li 
                    className="flex items-start gap-3"
                    whileHover={{ x: 5 }}
                  >
                    <Target className="w-5 h-5 flex-shrink-0 mt-1" style={{ color: colors.primario }} />
                    <span>Realizar diagnóstico organizacional y aplicar las propuestas correspondientes.</span>
                  </motion.li>
                  <motion.li 
                    className="flex items-start gap-3"
                    whileHover={{ x: 5 }}
                  >
                    <Target className="w-5 h-5 flex-shrink-0 mt-1" style={{ color: colors.primario }} />
                    <span>Diseñar e implementar planes de acción para la competitividad de la organización.</span>
                  </motion.li>
                </motion.ul>
              </div>
            </motion.section>
            ...
            <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-8 md:p-12 shadow-lg">
              ...
              >
                <motion.div 
                  ...
                  transition={{ duration: 0.6 }}
                >
                  <Briefcase className="w-7 h-7" style={{ color: colors.secundario }} />
                </motion.div>
                <motion.h2 
                  className="text-3xl font-bold text-foreground font-forte"
                  whileHover={{ scale: 1.02 }}
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Users className="w-5 h-5" style={{ color: colors.primario }} />
                    </motion.div>
                    <h3 className="text-lg font-bold text-foreground font-forte">Instituciones Públicas</h3>
                  </motion.div>
                  <p className="text-muted-foreground text-sm font-forte">
                    En dependencias y entidades integrantes de la administración pública, estatal, paraestatal, y municipal en actividades de: administración en alto nivel, recursos humanos, financieros, contraloría, mercadotecnia, relaciones públicas, diseño, implementación y evaluación de sistemas administrativos.
                  </p>
                </motion.div>
                ...
                >
                  ...
                  >
                    <motion.div 
                      ...
                      whileHover={{ rotate: -10 }}
                    >
                      <Briefcase className="w-5 h-5" style={{ color: colors.secundario }} />
                    </motion.div>
                    <h3 className="text-lg font-bold text-foreground font-forte">Instituciones Privadas</h3>
                  </motion.div>
                  <p className="text-muted-foreground text-sm font-forte">
                    En entidades del sector industrial, comercio y servicios como: administrador general, de recursos humanos, materiales y financieros, contraloría, mercadotecnia, relaciones públicas.
                  </p>
                </motion.div>
                ...
                >
                  ...
                  >
                    <motion.div 
                      ...
                      whileHover={{ y: -3 }}
                    >
                      <Award className="w-5 h-5" style={{ color: colors.terciario }} />
                    </motion.div>
                    <h3 className="text-lg font-bold text-foreground font-forte">Ejercicio Libre</h3>
                  </motion.div>
                  <p className="text-muted-foreground text-sm font-forte">
                    En ejercicio libre de su profesión como asesor y consultor en áreas administrativas, capacitación y desarrollo, mercadotecnia, finanzas, creación y promoción de empresas.
                  </p>
                </motion.div>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Target className="w-5 h-5" style={{ color: colors.primario }} />
                    </motion.div>
                    <h3 className="text-lg font-bold text-foreground font-forte">Emprendimiento</h3>
                  </motion.div>
                  <p className="text-muted-foreground text-sm font-forte">
                    Dueño de una empresa, aplicando todos los conocimientos adquiridos durante su formación profesional para crear y gestionar su propio negocio.
                  </p>
                </motion.div>
              </div>
            </div>
          ...
          <motion.section 
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden hover:shadow-2xl transition-all duration-300 p-8 md:p-12 mb-16"
          >
            <motion.h3 
