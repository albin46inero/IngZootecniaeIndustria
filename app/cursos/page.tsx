...
import { ArrowLeft, Calendar, MapPin, Clock, Users, DollarSign, MessageCircle, Search, ChevronLeft, ChevronRight, GraduationCap, Sparkles, Mail, AlertCircle } from 'lucide-react';
    ...
    const loadData = async () => {
      try {
        ...
        if (gacetaResponse.cursos && gacetaResponse.cursos.length > 0) {
          ...
          console.log('📋 Primer curso:', gacetaResponse.cursos[0]);
          
          setTodosCursos(gacetaResponse.cursos);
          
          // Filtrar solo CURSOS (excluir diplomados y seminarios)
          const cursosFiltrados = gacetaResponse.cursos.filter(
            (curso: Curso) => 
              curso.tipo_curso_otro?.tipo_conv_curso_nombre === "CURSOS" &&
              curso.det_estado === "1" &&
              !curso.det_titulo.toUpperCase().includes("DIPLOMADO") &&
              !curso.det_titulo.toUpperCase().includes("SEMINARIO")
          );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-white">
        ...
        >
          ...
          />
          <p className="text-muted-foreground font-medium">Cargando cursos...</p>
        </motion.div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background font-forte">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          >
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3">Cursos Académicos</h2>
            <p className="text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto">
              Capacítate con nuestros cursos especializados de la carrera de <span className="font-semibold" style={{ color: colors.primario }}>{institucionData?.institucion_nombre || 'Ciencias del Desarrollo'}</span>
            </p>
          ...
          {cursos.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
              {cursos.map((curso, index) => {
                const isBroken = brokenImages.has(curso.iddetalle_cursos_academicos.toString());
                ...
                return (
                  <motion.div
                    ...
                    whileHover={{ y: -8 }}
                    className="bg-white rounded-2xl shadow-sm border border-border overflow-hidden hover:shadow-xl transition group"
                  >
                    {/* 🔹 IMAGEN DEL CURSO CON MANEJO DE ERRORES */}
                    <div className="relative h-48 overflow-hidden bg-gradient-to-br from-[var(--color-primario)]/10 to-[var(--color-secundario)]/10">
                      {curso.det_img_portada && !isBroken ? (
                        <Image
                          src={getCursoImageUrl(curso.det_img_portada)}
                          alt={curso.det_titulo}
                          fill
                          className="object-cover"
                          onError={() => {
                            console.error('❌ Imagen de curso no encontrada:', {
                              id: curso.iddetalle_cursos_academicos,
                              titulo: curso.det_titulo,
                              path: curso.det_img_portada,
                              url: getCursoImageUrl(curso.det_img_portada)
                            });
                            setBrokenImages(prev => new Set(prev).add(curso.iddetalle_cursos_academicos.toString()));
                          }}
                        />
                      ) : (
                      ...
                      <div className="space-y-2 mb-6 pt-4 border-t border-border text-sm">
                        {curso.det_carga_horaria > 0 && (
                          <div className="flex items-center gap-2 text-muted-foreground">
                            ...
                            <span>{curso.det_carga_horaria} horas</span>
                          </div>
                        )}
                        {curso.det_modalidad && (
                          <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        {curso.det_fecha_ini && (
                          <div className="flex items-center gap-2 text-muted-foreground">
                            ...
                            <span>{formatDate(curso.det_fecha_ini)}</span>
                          </div>
                        )}
                        <div className="flex items-center gap-2 text-muted-foreground">
                          <DollarSign className="w-4 h-4" style={{ color: colors.primario }} />
                          <span>Bs. {curso.det_costo}</span>
                        </div>
                      </div>
                      {curso.det_grupo_whatssap && (
                        <motion.a
                          href={curso.det_grupo_whatssap.startsWith('http') ? curso.det_grupo_whatssap : `https://wa.me/591${curso.det_grupo_whatssap}`}
                          target="_blank"
